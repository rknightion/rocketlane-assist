...
def log_request_details(logger: logging.Logger, method: str, url: str, headers: Optional[dict] = None, params: Optional[dict] = None):
    ...
    if getattr(settings, 'debug_mode', False):
        ...
        if headers:
            ...
            if "api-key" in safe_headers:
                safe_headers["api-key"] = safe_headers["api-key"][:10] + "..." if safe_headers["api-key"] else "None"
            logger.debug(f"Headers: {safe_headers}")
        if params:
            logger.debug(f"Params: {params}")


def log_response_details(logger: logging.Logger, status_code: int, response_text: Optional[str] = None):
